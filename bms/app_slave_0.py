...
# Signal 1
tk.Label(master, text="Signal 1 (REQ: BMS -> NAS):").grid(row=0, column=0)
...
# Signal 2
tk.Label(master, text="Signal 2 (REQ: BMS -> NAS):").grid(row=1, column=0)
...
# Signal 3
tk.Label(master, text="Signal 3 (REQ: BMS -> NAS):").grid(row=2, column=0)
...
# Signal 4
# Label
tk.Label(master, text="Signal 4 (REQ: NAS -> BMS):").grid(row=3, column=0)
...
# Signal 5
tk.Label(master, text="Signal 5 (REQ: NAS -> BMS):").grid(row=4, column=0)
...
def get_bms_actual_set_rpm():
    ...
    o2.insert(tk.INSERT,"10 * RPM Setting:" + str(_to_int(hex2complement(bms_set_actual_rpm[0]),16)))
    o3.insert(tk.INSERT,"10 * RPM Actual:"  + str(_to_int(hex2complement(bms_set_actual_rpm[1]),16)))
    ...
    #print("Setting Point RPM:", bin(bms_set_actual_rpm[0]), "Twos compliment of Hex Number:", bin(bin(bms_set_actual_rpm[0]) & bin(0xffffffff)))
...
# - Ref: https://www.codegrepper.com/code-examples/python/how+to+convert+integer+to+hex+signed+2%27s+complement+in+python
...
def resp_nas_rpm_cmd():
    ...
    # Under Dev
    # gui_input = _to_int(hex2complement(10*gui_input),16)
    # nas_set_rpm = server.data_bank.set_input_registers(0,[gui_input])

    # (Deprecated)----------------------------------------------------------------
    nas_set_rpm = server.data_bank.set_input_registers(0,[int(10*int(gui_input))])
...
tk.Button(master, text="REQ (NAS->BMS)", width=10, command=show).grid(row=5, column=2, sticky= "w", padx=10, pady=5)
